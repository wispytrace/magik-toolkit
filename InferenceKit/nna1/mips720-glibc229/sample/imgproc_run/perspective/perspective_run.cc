...
int warp_perspective_run(int argc, char **argv) {
    /*set*/
    cpu_set_t mask;
    CPU_ZERO(&mask);
    CPU_SET(0, &mask);
    ...
    matrix_addr[0] = 0.96782;
    matrix_addr[1] = 0.15029;
    matrix_addr[2] = 13.5000;
    matrix_addr[3] = 0.01077;
    matrix_addr[4] = 1.13060;
    matrix_addr[5] = 15.0000;
    matrix_addr[6] = 0.00000;
    matrix_addr[7] = 0.00017;
    matrix_addr[8] = 1.00000;
    ...
    close(handle);
    /*perspective api*/
    ret = warp_perspective(src_img, dst_img, matrix, &param);

    write_bin_to_file("warp_perspective.bin", (char *)dst_addr, dst_size * sizeof(uint8_t));
...
int crop_perspective_run(int argc, char **argv) {
    /*set*/
    cpu_set_t mask;
    CPU_ZERO(&mask);
    CPU_SET(0, &mask);
    ...
    close(handle);
    /*perspective api*/
    ret = crop_perspective(src_img, dst_imgs, matrixs, infos, &param);
    for (int i = 0; i < box_num; i++) {
        char output_name[2048];
        sprintf(output_name, "./crop_perspective_%d.bin", i);
...
int main(int argc, char **argv) {
    printf("input number 0(warp_perspective_bgra)  1(crop_perspective_run):\n");
    int i = 0;
    scanf("%d", &i);
    if (i == 0)
        warp_perspective_run(argc, argv);
    else if (i == 1)
        crop_perspective_run(argc, argv);
    return 0;
}
