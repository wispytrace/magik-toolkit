...
Usage:
    $ python path/to/test.py --data coco128.yaml --weights yolov5s.pt --img 416
"""
...
def run(data,
    ...
    if training:  # called by train.py
        ...
        # model = Model(ckpt['model'].yaml, ch=3, nc=2).to(device)  # create
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            #    pred[:, 5] = 0
            #pred = pred[pred[:, 5] == 0]
            # if 'valid/face' in paths[si]:
            #     img_cls = 0
            #     pred = pred[pred[:, 5] == img_cls]
            # else:
            #     #img_cls = 1
            #     pred = pred[pred[:, 5] == 1]
            #     #pred2 = pred[pred[:, 5] == 2]
            #     #print(pred1)#var = var.cuda().data.cpu().numpy()
            #     #pred = np.vstack((pred1.data.cpu().numpy(),pred2.data.cpu().numpy()))
            #     #pred = torch.from_numpy(pred1).cuda()
            #pred = pred[pred[:, 5] == 0]
