...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['ema' if ckpt.get('ema') else 'model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        ...
        if 1:
            ...
            state_dict = [i for i in model.state_dict() if ("23" not in i)] #
            # print(state_dict)
            # exit()
            new_state_dict = {}
            # for (k1) in state_dict:
            #     print(k1)
            # for (k1, v) in pretrained_state_dict.items():
            #     print(k1)
            
            print(len(pretrained_state_dict.items()), len(state_dict))
            
            for (k, v),(k1) in zip(pretrained_state_dict.items(), state_dict):
                print('old--', k,"|", k1)
                new_state_dict[k1] = v
    ...
    if pretrained:
        ...
        # EMA
        #if EMA and ema and ckpt.get('ema'):
        #    ema.ema.load_state_dict(ckpt['ema'].float().state_dict())
        #    ema.updates = ckpt['updates']
            #part
            # ema.ema.load_state_dict(new_state_dict, strict=False)
            # ema.update(model)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Multi-scale
            if opt.multi_scale:
                sz = random.randrange(imgsz * 0.8125, imgsz * 1.25 + gs) // gs * gs  # size
            ...
            # Forward
            if 1:###with amp.autocast(enabled=cuda): ###hyu modify
...
def main(opt):
    ...
    if LOCAL_RANK != -1:
        ...
        dist.init_process_group(backend="nccl" if dist.is_nccl_available() else "gloo", timeout=timedelta(seconds=300))
