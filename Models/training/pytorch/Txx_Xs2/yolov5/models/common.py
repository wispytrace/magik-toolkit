...
class DWConv(nn.Module):
    # Depthwise convolution
    def __init__(self, c1, k=3, s=1, bitw8=False, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DWConv, self).__init__()
        if c1 > 12:
           assert c1%16 == 0
        self.dw = ops.DepthwiseConv2D(in_channels=c1,
                                      stride=s,
                                      kernel_h=k,
                                      kernel_w=k,
...
class DW_Conv(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, bitw8=False, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(DW_Conv, self).__init__()
        self.dw = DWConv(c1, k=k, s=s)
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, bitw8=False, p=None, g=1, act=True, first_layer=False, is_focus=False):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # self.act = activation_fn if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        #print(c1, c2)
        if c1 > 12:
           assert c1%16 == 0
           assert c2%16 == 0
        self.conv = ops.Conv2D(in_channels=c1,
                               out_channels=c2,
                               kernel_h=k,
                               kernel_w=k,
                               stride=s,
                               activation_fn=activation_fn,#act
                               enable_batch_norm=True,
                               enable_bias=False,
                               padding= autopad(k, p),
                               first_layer=first_layer,
                               quantize= is_quantize,
                               weight_bitwidth= int(8 if (bitw8 and bitw!=32) else bitw),
                               input_bitwidth=int(8 if first_layer==1 else bita),
                               output_bitwidth=int(8 if (bitw8 and bita!=32) else bita),#bita,
                               weight_factor=weight_factor,
                               clip_max_value=clip_max_value,
                               groups = 1,
                               is_focus = is_focus,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        #self.cv2 = GConv(c_, c2, 3, 1, g=g)
        #if c2 > 64:
        #    self.cv2 = GConv(c_, c2, 3, 1, g=g)
        #else:
        #    self.cv2 = Conv(c_, c2, 3, 1)
        self.cv2 = DW_Conv(c_, c2, 3, 1)
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = ops.Conv2D(in_channels=c1,
                              ...
                              stride=1,
                              activation_fn=activation_fn,#nn.LeakyReLU(0.1, inplace=True),
                              enable_batch_norm=True,
                              enable_bias=False,
                              padding= 0,##autopad(1, 1),
                              first_layer=0,
                              last_layer =0,
                              quantize= is_quantize,
                              weight_bitwidth=bitw,
...
class GConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        assert c1%32 == 0
        assert c2%32 == 0
        self.conv1 = ops.Conv2D(in_channels=c1,
                                out_channels=c1,
                                ...
                                groups =c1//32,
                                target_device = target_device)
        self.conv2 = ops.Conv2D(in_channels=c1,
                                out_channels=c2,
                                kernel_h=1,kernel_w=1,
                                stride=1,
                                activation_fn=activation_fn,
...
class GroupNet(nn.Module):
    ...
    def forward(self, x):
        # res = self.cv1(x)
        # return res
        return self.Shortcut([x, self.cv1(x)]) if self.add else self.cv1(x)
...
class GroupNetCSP(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=True, g=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(GroupNetCSP, self).__init__()
        self.cv1 = GConv(c1, c1, 3, 1)
        self.cv2 = Conv(2 * c1, c2, 1, 1)
        self.m = nn.Sequential(*[GroupNet(c1, c1, shortcut, g) for _ in range(n)])

    def forward(self, x):
        y1 = self.m(x)
        y2 = self.cv1(x)
        # return self.cv2(torch.cat((y1, y2), dim=1))
        res = cat_modules([y1, y2])
        res = self.cv2(res)
        return res
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        ...
        return self.conv(x)  ##focus used in conv
...
class Conv_0(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=2, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = Conv(c1, c2, k, s, p, g, act, first_layer=True)
...
class Maxpool(nn.Module):
    def __init__(self, c1, s=2, padding=1):
        super(Maxpool, self).__init__()
        self.maxpool = ops.Maxpool2D(kernel_h=c1, kernel_w=c1, stride=s, padding=padding, target_device=target_device)
