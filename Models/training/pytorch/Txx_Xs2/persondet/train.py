...
wdir = 'weights/32bit' + os.sep  # weights dir
last = wdir + 'last.pt'
best = wdir + 'best.pt'
...
hyp = {'giou': 3.54,  # giou loss gain
       ...
       'degrees': 10 * 0, #0.0, #15 * 1,  # image rotation (+/- deg)
       'translate': 0.05 * 0, #0.0,#0.1 * 1,  # image translation (+/- fraction)
...
def train():
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        if 'scheduler' in chkpt:
            if chkpt['scheduler'] is not None and opt.resume:
                scheduler.load_state_dict(chkpt['scheduler'])
        if 'ema_updates' in chkpt:
            # load ema.updates
            if chkpt['ema_updates'] is not None and opt.resume:
                ema_uqdates = chkpt['ema_updates']
            
        if opt.resume:
            start_epoch = chkpt['epoch'] + 1
        have_load_model = True
        del chkpt
    ...
    if device.type != 'cpu' and torch.cuda.device_count() > 1 and torch.distributed.is_available():
        dist.init_process_group(backend='nccl',  # 'distributed backend'
                                init_method='tcp://127.0.0.1:8887',  # distributed training init method
    ...
    testloader = torch.utils.data.DataLoader(LoadImagesAndLabels(test_path, img_size_test, batch_size,
                                                                 hyp=hyp,
                                                                 rect=True), #cache_images=opt.cache_images),
                                             batch_size=batch_size,
                                             num_workers=nw,
                                             pin_memory=True,
                                             collate_fn=dataset.collate_fn)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        print(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'lr', 'GIoU', 'obj', 'cls', 'total', 'targets', 'img_size'))
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            s = ('%10s' * 2 + '%10.5g'  + '%10.3g' * 6) % ('%g/%g' % (epoch, epochs - 1), mem, lr,  *mloss, len(targets), img_size)
