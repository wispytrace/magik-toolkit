...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for k, v in model.named_modules():
        ...
        if "BatchNorm2d" in k and hasattr(v, 'weight'):#isinstance(v, nn.BatchNorm2d):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Multi-scale
            if opt.multi_scale:
                sz = random.randrange(imgsz * 0.625, imgsz * 1.0 + gs) // gs * gs  # size
            ...
            # Forward
            if 1:###with amp.autocast(enabled=cuda):
        ...
        if RANK in [-1, 0]:
            ...
            if not notest or final_epoch:  # Calculate mAP
                ...
                results, maps, _ = test.run(data_dict,
                                            ...
                                            imgsz=imgsz_test,
                                            model=ema.ema if use_ema else model,
